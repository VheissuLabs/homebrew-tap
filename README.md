...
single binary with its own PHP built in, so it needs nothing else on the machine. Apple Silicon
Macs, and Linux on x86_64 and ARM. `brew upgrade tql` moves to a new release and `brew uninstall tql`
